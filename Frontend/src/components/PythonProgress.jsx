...
export default function PythonProgress() {
  ...
  async function fetchTopics(signal) {
    ...
    setErr("");
    try {
      const res = await fetch(`/progress/python/topics?clerk_id=${encodeURIComponent(user.id)}`, { signal });
  ...
  async function markComplete(key) {
    ...
    try {
      const res = await fetch(`/progress/python/complete`, {
        method: "POST",
  ...
  async function resetAll() {
    if (!user?.id) return;
    try {
      const res = await fetch(`/progress/python/reset`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  if (!isSignedIn) {
    return (
      <div className="container" style={{ padding: 20 }}>
        <p>Please sign in to view Python progress.</p>
  ...
  return (
    <div className="container" style={{ padding: 20 }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ margin: 0 }}>Python Course Progress</h1>
